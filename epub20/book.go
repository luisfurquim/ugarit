...
func NewIndexGenerator(lang string, uid string, tit string, auth string, b ugarit.Book) (*IndexGenerator, error) {
   ...
   ig.curr = []*[]NavPoint{&ig.doc.Points}
   return &ig, nil
}
...
func (gen *IndexGenerator) AddItem(item *html.Node) error {
   ...
   if len(gen.curr)<2 {
      ...
      ptr := gen.curr[len(gen.curr)-2]
      if (*ptr)[len(*ptr)-1].Content.Src != item.Attr[0].Val {
         gen.id++
      } else {
         id = "a"
      }
   ...
   *gen.curr[len(gen.curr)-1] = append(*gen.curr[len(gen.curr)-1], NavPoint{
      Id:        fmt.Sprintf("pg%d%s",gen.id,id),
      PlayOrder: fmt.Sprintf("%d",gen.id),
      Label:     label,
      Content: Content{
         Src: item.Attr[0].Val,
      },
...
func (gen *IndexGenerator) AddSection() error {
   ptr := gen.curr[len(gen.curr)-1]
   gen.curr = append(gen.curr,&(*ptr)[len(*ptr)-1].Points)
//   fmt.Printf("%#v\n",gen.curr)
   return nil
}
