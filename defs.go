...
type Book interface {
   ...
   // AddReference just register a file as having the provided mimetype, without
   // storing its content in the eBook.
   // If path collides with a reserved path, it must return an error.
   AddReference(path string, mimetype string, id string, options interface{}) (string, error)
...
type IndexGenerator interface {
   ...
   // E.G. EPub 3.0 returns 'nav' and EPub 2.0 returns ''
   GetPropertyValue() string

   // GetId returns the Id value for the TOC file in the manifest
   // E.G. EPub 3.0 returns 'nav' and EPub 2.0 returns 'ncx'
   GetId() string
}

type TOC interface {
   TOCChild(n int) TOCRef // Compatibility break: it previously was "TOCChild(n int) TOC"
   TOCLen() int
}
...
type Versioner interface {
   // The versioner compliant object must provide a Next method
...
var ErrorAlreadyTopLevel = errors.New("Error already top level")
